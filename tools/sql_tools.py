...
class Sqlmap(HackingTool):
    TITLE = "Sqlmap tool - Detecting and exploiting SQL injection flaws and taking"
...
class NoSqlMap(HackingTool):
    TITLE = "NoSqlMap - Audit for as well as automate injection attacks and exploit"
...
class SQLiScanner(HackingTool):
    TITLE = "Damn Small SQLi Scanner - DSSS is a fully functional SQL"
...
class Explo(HackingTool):
    TITLE = "Explo - simple tool to describe web security issues"
...
class Blisqy(HackingTool):
    TITLE = "Blisqy - Exploit Time-based blind-SQL injection Web Security researchers to find "
...
class Leviathan(HackingTool):
    TITLE = "Leviathan - Wide Range Mass Audit Toolkit brute force, SQL injection detection"
...
class SQLScan(HackingTool):
    TITLE = "SQLScan - Quick web scanner for find an sql inject point"
