...
class WIFIPumpkin(HackingTool):
    TITLE = "WiFi-Pumpkin - rogue AP framework to easily create"
...
class pixiewps(HackingTool):
    TITLE = "pixiewps - Tool written in C used to bruteforce offline"
...
class BluePot(HackingTool):
    TITLE = "Bluetooth Honeypot GUI Framework - You need to have at least 1 bluetooh receiver"
...
class Fluxion(HackingTool):
    TITLE = "Fluxion - Remake of linset by vk496 with enhanced functionality"
...
class Wifiphisher(HackingTool):
    TITLE = "Wifiphisher - victim-customized web phishing attacks against"
    DESCRIPTION = """
...
class Wifite(HackingTool):
    TITLE = "Wifite - Automated wireless attack tool"
...
class EvilTwin(HackingTool):
    TITLE = "EvilTwin - Credentials using a Fake page and Fake Access Point"
...
class Fastssh(HackingTool):
    TITLE = "Fastssh - brute force attack against SSH protocol"
...
class Howmanypeople(HackingTool):
    TITLE = "Howmanypeople - Number of people around you by monitoring wifi"
