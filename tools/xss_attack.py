...
class Dalfox(HackingTool):
    TITLE = "DalFox - Scanning and Parameter Analysis tool"
...
class XSSPayloadGenerator(HackingTool):
    TITLE = "XSS Payload Generator - XSS PAYLOAD GENERATOR -XSS SCANNER-XSS DORK FINDER"
...
class XSSFreak(HackingTool):
    TITLE = "XSS-Freak - XSS scanner fully written in python3 from scratch"
...
class XSpear(HackingTool):
    TITLE = "XSpear - XSS Scanner on ruby gems"
...
class XanXSS(HackingTool):
    TITLE = "XanXSS - reflected XSS searching tool"
...
class XSSStrike(HackingTool):
    TITLE = "Advanced XSS Detection Suite - python script designed to detect and exploit XSS vulnerabilites"
...
class RVuln(HackingTool):
    TITLE = "RVuln - Multi-threaded and Automated Web Vulnerability"
