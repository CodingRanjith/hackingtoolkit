...
class TheFatRat(HackingTool):
    TITLE = "The FatRat - TheFatRat Provides An Easy way to create Backdoors"
...
class Brutal(HackingTool):
    TITLE = "Brutal - quickly create various payload"
...
class Stitch(HackingTool):
    TITLE = "Stitch - Cross Platform Python Remote Administrator Tool"
...
class MSFVenom(HackingTool):
    TITLE = "MSFvenom Payload Creator - (MSFPC) is a wrapper to generate"
...
class Venom(HackingTool):
    TITLE = "Venom Shellcode Generator - apache2 webserver to deliver payloads using a fake webpage"
...
class Spycam(HackingTool):
    TITLE = "Spycam - Script to generate a Win32 payload that takes the webcam"
...
class MobDroid(HackingTool):
    TITLE = "Mob-Droid - helps you to generate metasploit payloads"
...
class Enigma(HackingTool):
    TITLE = "Enigma - Multiplatform payload dropper"
